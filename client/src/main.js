...
const requestMatchUrl = "http://localhost:5500/routes/api/matches/EUW1_6942386737";
let matchResult = axios.get(requestMatchUrl).then(results => {
    ...
    const thisMatchData = results.data.info;
    ...
    Promise.all([itemResult,timelineResult,voiceDataResult,stopWordsResult,sentimetsResult]).then(values => {
...
Promise.all([matchResult]).then(values => {
